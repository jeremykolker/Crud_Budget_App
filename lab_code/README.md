...
- see a list of your income and expenditures (using Mongo/Mongoose)
- show you one income/expenditure item (using Mongo/Mongoose)
- create a new income/expenditure item (using Mongo/Mongoose)
...
## Data
You are provided a `budget.js` file with data for you to use to populate your index and show routes (use this data for your seed route. Either copy paste the array into an insertMany or you can export the array and import it into wherever your seed route is.)
...
- Models
  - Create a Schema and Mongoose Model for the budgets that match the provided seed data in `budget.js`.
- Controllers
...
### Database Setup
- Create a schema/model for your budgets (`Budget.js`) that matches the provided data in `budget.js`.
- Make sure you configure your server to use Mongo and provide a sub database for it to connect to (look back at the mongo day notes if you need a refresher).
- Export the mongoose model for use in your routes.

### Seed
- Use the data provided in `budget.js` to create a seed route (maybe have this hit /seed or /index/seed its up to you) that will populate your database with initial values.

#### Index
- Set your` models/Budget.js` (the model you created not the seed data provided) data to a variable named `Budget`
...
- once the `req.body` is what you'd an expect (an object with keys that match our data in our `models/budget.js` and values that were entered in your form), add `req.body` to your collection using the Mongoose `.create()` method.
...
> **STRETCH GOAL!** Make a middleware route that does all of the above. It should calculate the bankAccount total and store it in a variable called [`res.locals.sum`](https://expressjs.com/en/api.html#res.locals) that you can now access in all routes, including the index route where you should display the total on the `index.ejs` page. 
